...
for p2=1:length(Par2)
    ...
    parfor k=1:NumIter
        ...
        Sus(k,:) = Ns;
        %
        Inf_1(k,:) = Ninf(:,1); % time series of the number of infected with starin 1 for iteration k
        Inf_2(k,:) = Ninf(:,2); % time series of the number of infected with starin 2 for iteration k
...
am = mean(a);
am2 = mean(a.^2);
Thld = (mu1/m)*(1/(am+sqrt(am2)));

% theorem 2
Xv = Par2;
Xv(Xv<=Thld) = [];
% 
Falpha = (mu1./(Xv)) * ((1/m).*1./(am+sqrt(am2))) ;
...
s.EdgeColor = 'none';
hold on
plot3(Thld.*(ones(1,length(Par2)) ),Par1,hh*(ones(1,length(Par2))),'--w','LineWidth',3)
...
s.EdgeColor = 'none';
hold on
plot3(Xv,Falpha,hh*(ones(1,length(Xv))),'--w','LineWidth',3)
plot3(Thld.*(ones(1,length(Par2)) ),Par1,hh*(ones(1,length(Par2))),'--w','LineWidth',3)
